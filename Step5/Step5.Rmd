...
# Load transcriptomics data
 url <- "http://perfettolab.bio.uniroma1.it/Step5/TCGA/transcriptomics_TCGA.tsv"
      response <- httr::GET(url)
      tras <- readr::read_tsv(content(response, "text"))
...
for (analysis_name in analysis_name_list) {
  ...
  # Load transcriptomics data and compute mean expression for each gene
   url_zscored <- "http://perfettolab.bio.uniroma1.it/Step5/TCGA/transcriptomics_TCGA.tsv"
      response <- httr::GET(url_zscored)
      omic_CPTAC <- readr::read_tsv(content(response, "text"))
