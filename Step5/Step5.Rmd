...
NMF <- NMF %>%
  filter(!is.na(final_score), !is.na(community)) %>%  # Remove missing values
  filter(!gene_name %in% c("TELOMERE MAINTENANCE")) %>%  # Exclude specific genes
  mutate(
...
p <- ggplot(NMF, aes(x = gene_name, y = final_score_transformed, fill = community)) +
