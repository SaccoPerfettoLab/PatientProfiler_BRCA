...
```{r Table2 generation, eval = FALSE}

clinical_dataframe <- read_tsv("../Step1/clinical_dataframe.tsv")
rownames(clinical_dataframe) <- clinical_dataframe$Name
clinical_dataframe.T <- as.data.frame(t(clinical_dataframe))[-1, ]
clinical_dataframe.T <- clinical_dataframe.T[, c("Replicate_Measurement_IDs","NMF.Cluster")]  # Solo nmf_cluster
clinical_dataframe.T$patient_ID <- rownames(clinical_dataframe.T)

pat_names <- clinical_dataframe.T$patient_ID

Tabella_totale_risk <- bind_rows(lapply(seq_along(pat_names), function(i) {
  message(i)
  patient_id <- pat_names[i]
  
  path_constraints <- paste0("./Activities/Activity_constraints_Patient_", patient_id, ".tsv")
  path_standard <- paste0("./Activities/Activity_Patient_", patient_id, ".tsv")
  
  if (file.exists(path_constraints)) {
    pat_data <- read_tsv(path_constraints)
  } else if (file.exists(path_standard)) {
    pat_data <- read_tsv(path_standard)
  } else {
    return(NULL)
  }
  ...
  pat_data <- cbind(Replicate_Measurement_IDs = rep(replicate_id, nrow(pat_data)),
                    NMF.Cluster = rep(nmf_cluster, nrow(pat_data)),
                    pat_data)
  return(pat_data)
}))

Tabella_totale_risk <- distinct(Tabella_totale_risk)


write_xlsx(Tabella_totale_risk, "Table_2.xlsx")
